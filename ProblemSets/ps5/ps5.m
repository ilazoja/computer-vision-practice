...
f = figure;
quiver(U,V,50);
saveas(f, fullfile('output', 'ps5-4-a-TestSeq-1-1.png'));
warpedFrame = warp(double(TestSeq(:,:,4)),U,V);
ogFrame = double(TestSeq(:,:,1));
diff = uint8(abs(ogFrame - warpedFrame));
imwrite(diff, fullfile('output', 'ps5-4-a-TestSeq-2-1.png'));
...
ogFrame = double(TestSeq(:,:,1));
diff = uint8(abs(ogFrame - warpedFrame));
imwrite(diff, fullfile('output', 'ps5-4-a-TestSeq-2-2.png'));

[U,V] = hierarchicalLK(DataSeq1(:,:,1),DataSeq1(:,:,2), 3); 
f = figure;
quiver(U,V,50);
saveas(f, fullfile('output', 'ps5-4-b-1-2.png'));
warpedFrame = warp(double(DataSeq1(:,:,1)),U,V);
...
f = figure;
quiver(U,V,50);
saveas(f, fullfile('output', 'ps5-4-a-DataSeq1-1-1.png'));
warpedFrame = warp(double(DataSeq1(:,:,2)),U,V);
ogFrame = double(DataSeq1(:,:,3));
diff = uint8(abs(ogFrame - warpedFrame));
imwrite(diff, fullfile('output', 'ps5-4-b-2-2.png'));
